...
public class PasswordQualityChecker {
    ...
    private static HashSet<String> passwordList;
    private static int minPasswordLength;

    public PasswordQualityChecker(int minPassLength) {
        this.minPasswordLength = minPassLength;
        this.passwordList = new HashSet<>(10000);

        try {
            InputStream stream = getClass().getClassLoader().getResourceAsStream(COMMON_PASSWORDS_TXT);
     ...
     */
    public static void checkPassword(String password) throws PasswordTooCommonException, PasswordTooShortException {
